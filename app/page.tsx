...
// ── Store XR (singleton, di luar komponen) ───────────────────
...
// Reticle + hit-test — relativeTo 'viewer' (pusat kamera)
function HitTestManager({
  ...
  // useXRHitTest(callback, relativeTo) — relativeTo wajib di v6
  useXRHitTest(
    ...
    "viewer" // ← argumen ke-2 wajib: sumber ray hit-test
  );
...
function ARScene({
  ...
  // v6: gunakan useXR dengan selector untuk cek session aktif
  const session = useXR((state) => state.session);
  const isPresenting = session != null;

  // Expose place handler ke parent lewat window
  ...
  return (
    <>
      ...
      {/* Preview 3D saat belum masuk AR */}
      {!isPresenting && (
      ...
      {/* Saat AR aktif: reticle + objek yang sudah ditempatkan */}
      {isPresenting && (
        <>
...
export default function Home() {
  ...
  return (
    ...
    >
      ...
      {/* Tombol place — hanya muncul saat hit surface terdeteksi */}
      {currentHit && (
        <button
          onClick={handlePlace}
          style={{
      ...
      >
        <XR store={store}>
          <ARScene onHitUpdate={setCurrentHit} />
        </XR>
      </Canvas>
    </main>
  );
